...
echo "=== MT7927 WiFi-Only restoration with DIS_MAT Handshake Fix ==="
...
if [ -n "$W_PATCH" ] && [ -n "$W_RAM" ]; then
    ...
    echo "WiFi Firmware (MT7927) successfully placed."
else
    echo "!!! ERROR: Firmware files not found in ./firmware/wifi/"
    exit 1
fi
...
# DIS_MAT fix: Exempts EAPOL (handshake) frames from hardware address translation
sudo sed -i 's/q_idx <= MT_LMAC_BCN0)/q_idx <= MT_LMAC_BCN0) || skb->protocol == cpu_to_be16(ETH_P_PAE)/' "${DKMS_DIR}/mt76/mt7925/mac.c"

sudo cp mt6639-wifi-init.patch mt6639-wifi-dma.patch mt7902-wifi-6.19.patch "${DKMS_DIR}/"
cd "${DKMS_DIR}/mt76"
...
# Flex-buffer fix for Kernel 6.19
...
sudo sed -i '/mt7925e/d' /etc/initramfs-tools/modules
sudo update-initramfs -u

echo "=================================================================="
echo "SUCCESS! WiFi-only driver with DIS_MAT fix is restored."
echo "Perform a standard REBOOT now."
echo "=================================================================="
